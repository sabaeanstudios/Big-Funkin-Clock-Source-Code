...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (songLowercase)
		{
			...
			case 'overtime':
				dialogue = CoolUtil.coolTextFile(Paths.txt('overtime/dialog'));
			case 'crunch':
				dialogue = CoolUtil.coolTextFile(Paths.txt('crunch/dialog'));
			case 'clocked':
				dialogue = CoolUtil.coolTextFile(Paths.txt('clocked/dialog'));
		}
		...
		if (SONG.stage == null) {
			switch(storyWeek)
			{
				...
				case 5: if (songLowercase == 'thorns') {stageCheck = 'schoolEvil';} else {stageCheck = 'school';}
			...
			case 'clockAngry':
				{
					...
					var bg:FlxSprite = new FlxSprite(-100).loadGraphic(Paths.image('clockAngry/space', 'shared'));
					bg.scrollFactor.set(0.1, 0.1);
					add(bg);

					var city:FlxSprite = new FlxSprite(-10).loadGraphic(Paths.image('clockAngry/moon', 'shared'));
					city.scrollFactor.set(0.3, 0.3);
					city.setGraphicSize(Std.int(city.width * 0.85));
					city.updateHitbox();
					add(city);

					phillyTrain = new FlxSprite(2000, 360).loadGraphic(Paths.image('clockAngry/car','shared'));
					if(FlxG.save.data.distractions){
						add(phillyTrain);
					}

					var street:FlxSprite = new FlxSprite(-40, -4).loadGraphic(Paths.image('clockAngry/street','shared'));
					add(street);
			...
			case 'clockMeh':
				{
					...
					var bg:FlxSprite = new FlxSprite(-100).loadGraphic(Paths.image('clockAngry/spacePurp', 'shared'));
					bg.scrollFactor.set(0.1, 0.1);
					add(bg);
			...
			case 'clockHappy':
				{
					...
					var bg:FlxSprite = new FlxSprite(-100).loadGraphic(Paths.image('clockAngry/spaceBlue', 'shared'));
					bg.scrollFactor.set(0.1, 0.1);
					add(bg);
		...
		healthBar.createFilledBar(0xFFFF0000, 0xFF00b7e0);
		
		if(SONG.song.toLowerCase()=='clocked' || SONG.song.toLowerCase()=='crunch' || SONG.song.toLowerCase()=='overtime'  )
		{
				healthBar.createFilledBar(0xFF987654, 0xFF00b7e0);
		}
		// healthBar
		add(healthBar);
		...
		else
		{
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					{
					case 'overtime':
						LoadingState.loadAndSwitchState(new VideoState("assets/videos/CutsceneEnd.webm", new MainMenuState()));
					default:
